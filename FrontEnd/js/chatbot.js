/**
 * Widget chatbot Toko Sembako — kanan bawah, terkunci jika belum login.
 * Memanggil POST /api/chat via Flask → 9Router (OpenAI-compatible).
 */
(function () {
    let chatOpen = false;
    let chatHistory = [];
    ...
    async function sendChatMessage() {
        if (typeof isLoggedIn === "function" && !isLoggedIn()) {
            window.location.href = "login.html?return=" + encodeURIComponent(window.location.pathname.split("/").pop());
            return;
        }
        const input = document.getElementById("chatInput");
        ...
        const errMsg = res.data?.error || "Gagal membalas. Periksa Flask & konfigurasi 9Router.";
        chatHistory.push({
            role: "assistant",
    ...
    async function loadProviderLabel() {
        ...
        const names = { nvidia: "NVIDIA NIM", openrouter: "OpenRouter", "9router": "9Router" };
        el.textContent = "Powered by " + (names[res.data.provider] || res.data.provider);
    }
